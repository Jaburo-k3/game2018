...
public class equip_weapon : MonoBehaviour {
    ...
    void set_weapon(int arm_number,int number,int slot_number) {
        if (arm_number == 0)
        {
            E_weapon[arm_number, slot_number] = Instantiate(R_weapon[assemble_status.my_weapon_number[number]]);
        }
        else{
            E_weapon[arm_number, slot_number] = Instantiate(L_weapon[assemble_status.my_weapon_number[number]]);
        }
        Weapon_Status = E_weapon[arm_number,slot_number].GetComponent<weapon_status>();
